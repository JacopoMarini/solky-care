    ...
    <div className="min-h-screen bg-muted/30">
            ...
            <Card>
              <CardContent className="flex flex-wrap items-center justify-between gap-3 py-4">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={openDetail}>Dettaglio giornaliero</Button>
                        ...
                        {rows.map((r) => (
                          <TableRow key={r.locationName}>
                            <TableCell className="flex items-center gap-2">
                              <MapPin className="h-3 w-3 text-muted-foreground" /> {r.locationName}
                            </TableCell>
                ...
                <CardContent className="space-y-2">
                  {users.filter((u) => u.status === 'pending').map((u) => (
                    <div key={u.id} className="flex items-center gap-3 rounded-lg border border-amber-200 bg-white px-3 py-2.5">
                      ...
                      <Button size="sm" className="h-8 gap-1 shrink-0" onClick={() => activateUser(u)} disabled={activatingId === u.id}>
                        <UserCheck className="h-3.5 w-3.5" />
                        {activatingId === u.id ? 'Abilitando…' : 'Abilita'}
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive shrink-0" onClick={() => confirmDelete({ type: 'user', id: u.id, label: u.name })}>
            ...
            <Card>
              <Table>
                ...
                <TableBody>
                  {users.filter((u) => u.status !== 'pending').length === 0 && (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center text-muted-foreground py-6">Nessun utente attivo</TableCell>
                    </TableRow>
                  )}
                  {users.filter((u) => u.status !== 'pending').map((u) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          {u.id !== user?.id && (
                            <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => confirmDelete({ type: 'user', id: u.id, label: u.name })}>
        ...
        {tab === 'locations' && (
          <>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex gap-2">
                  <Input placeholder="Es. Villa Bianca" value={newLocName} onChange={(e) => setNewLocName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addLocation()} />
            ...
            <Card>
              <Table>
                ...
                <TableBody>
                  {locations.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={2} className="text-center text-muted-foreground py-6">Nessuna location presente</TableCell>
                    </TableRow>
                  )}
                  {locations.map((loc) => (
                    <TableRow key={loc.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => confirmDelete({ type: 'location', id: loc.id, label: loc.name })}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </TableCell>
                    </TableRow>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deleteTarget?.type === 'location'
                ? <>Vuoi eliminare la location <strong>&ldquo;{deleteTarget.label}&rdquo;</strong>? Le ore già registrate rimarranno nel database.</>
                : <>Vuoi eliminare l&apos;utente <strong>{deleteTarget?.label}</strong>? Tutti i suoi dati verranno eliminati.</>
              }
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting}>Annulla</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={handleConfirmedDelete} disabled={deleting}>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Dettaglio — {MONTHS_IT[currentMonth.month - 1]} {currentMonth.year}</DialogTitle>
          </DialogHeader>
          <Table>
            <TableHeader>
              <TableRow>
