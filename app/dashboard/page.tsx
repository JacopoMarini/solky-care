      ...
      <main className="mx-auto max-w-2xl px-4 py-5 space-y-4">
        ...
        <div className="rounded-xl border bg-background shadow-sm overflow-hidden">
          <div className="grid grid-cols-7 border-b">
            {DAYS_IT.map((d, i) => (
              <div key={d} className={cn('py-2 text-center text-xs font-medium text-muted-foreground', i >= 5 && 'text-rose-400')}>
                {d}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7">
            {cells.map((day, idx) => {
              if (!day) return <div key={`empty-${idx}`} className="border-b border-r p-1 min-h-[80px] bg-muted/20 last:border-r-0" />;
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className={cn(
                      ...
                      isToday ? 'bg-primary text-primary-foreground' : colIdx >= 5 ? 'text-rose-500' : 'text-foreground'
                    )}>
                      {day}
                    </span>
                    {clickable && !hasEntries && <Plus className="h-3.5 w-3.5 text-muted-foreground/60" />}
                    {hasEntries && <span className="text-[10px] font-semibold text-primary">{formatHours(dayTotal)}</span>}
                  </div>
                  <div className="flex flex-col gap-0.5">
                    {dayEntries.map((e) => {
                      const color = colorByLocId[e.locationId] ?? LOC_COLORS[0];
                      return (
                        <div key={e.id} className={cn('flex items-center gap-1 rounded px-1 py-0.5', color.bg)}>
                          <div className={cn('h-1.5 w-1.5 rounded-full flex-shrink-0', color.dot)} />
                          <span className={cn('text-[9px] font-medium leading-tight truncate', color.text)}>{e.locationName}</span>
                        </div>
                      );
                    })}
                  </div>
      ...
      <Dialog open={!!modalDate} onOpenChange={(open) => !open && setModalDate(null)}>
        <DialogContent className="max-w-md w-full flex flex-col max-h-[90vh]">
                ...
                {locations.map((loc, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {loc.name}
                      {count > 1 && <span className="rounded-full bg-white/40 px-1.5 text-xs font-bold">{count}</span>}
                    </button>
                  );
                })}
              </div>
                    ...
                    <div key={slot.key} className={cn('rounded-lg p-3 space-y-2', color.bg)}>
                      <div className={cn('flex items-center gap-1.5 text-sm font-semibold', color.text)}>
                        ...
                        <div className="ml-auto flex items-center gap-1">
                          {slot.hours > 0 && <Badge className="text-xs py-0" variant="secondary">{formatHours(slot.hours)}</Badge>}
                          <button type="button" onClick={() => removeSlot(slot.key)} className="opacity-60 hover:opacity-100 transition-opacity">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <Input type="time" value={slot.startTime} onChange={(e) => updateSlot(slot.key, 'startTime', e.target.value)} className="h-8 text-sm" />
                        </div>
                        <div>
                          <label className="text-[10px] font-medium text-muted-foreground block mb-1">Fine</label>
                          <Input type="time" value={slot.endTime} onChange={(e) => updateSlot(slot.key, 'endTime', e.target.value)} className="h-8 text-sm" />
                        </div>
                      </div>
                      {slot.startTime && slot.endTime && slot.hours === 0 && (
                        <p className="text-[10px] text-rose-500">L'orario di fine deve essere dopo l'inizio</p>
                      )}
                      <Input placeholder="Note (opzionale)" value={slot.notes} onChange={(e) => updateSlot(slot.key, 'notes', e.target.value)} className="h-8 text-sm" />
                    </div>
                  );
                })}
                {modalTotal > 0 && (
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setModalDate(null)}>Annulla</Button>
            <Button onClick={handleSave} disabled={saving || slots.length === 0 || modalTotal === 0}>
